chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
  if (request.action === 'applyChanges') {
    applyChanges(request.selectedFontFamily, request.boldLetters);
  }
});

function applyChanges(selectedFontFamily, boldLetters) {
  document.querySelectorAll('*').forEach(element => {
    ...
    if (textContent) {
      const newTextContent = textContent.replace(/[szpb]/gi, match => {
